...
- 建议开启 AutoJs6 通知权限；任务会通知“领取成功、今日已领取或领取失败”。
...
领取点击后，脚本以“50 提现免费券”详情弹窗作为成功依据。优先识别“领取后365天内可用”“已领取，回首页”等专属文案；若小程序 WebView 不暴露文字，则通过 Shizuku 临时截屏检测详情弹窗的视觉结构。临时图片在检测后立即删除。5 秒内未出现该弹窗即判定失败，发送失败通知且不会重复点击。
